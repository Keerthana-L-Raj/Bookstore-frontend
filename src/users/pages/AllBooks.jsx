...
try{
 ...
 console.log(result);
  if (result.status === 200) {
      setAllBooks(result.data); // <-- This is critical
      setTempData(result.data)//assign book details to temp state
    } else {
            ...
            <div className="sm:col-span-4">
              ...
              <div className="mt-5 mx-20">
                <div className="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                  ...
                  />
                  <Button  className="flex flex-wrap gap-2 !bg-amber-950">Search</Button>
                </div>
              </div>
            </div>
             </div>
...
<div className='flex mt-20'>
  <div className='w-64 ms-30 mb-8 flex-none...'>
    ...
    <div>
      <input type="radio" onClick={()=>handleFilter('Literary')} id='Literary' name='filter' className='mb-4 me-1' />
          ...
          
            {
              ...
              <div className='"py-4'hidden={item.status=='pending' || item?.status=='sold'}>
               <Link to={`/viewBook/${item._id}`}>
                <Card className=" flex flex-wrap !bg-amber-100 h-100 w-70 p-1 text-center"   horizontal>
